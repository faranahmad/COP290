...
void ShowMissile(Bullet &b)
{
	...
	// Showing the smoke at the end of the missile
	glPushMatrix();
...
void ShowLives()
{
	// Displaying the lives and missiles
...
{
	float v = (15.0 * ((GLfloat) rand ()) / ((GLfloat) RAND_MAX)) - 7.5;
	float theta1= (PI* (((GLfloat) rand ()) / ((GLfloat) RAND_MAX)));
	float x= v*cos(theta1);
	float y= v*sin(theta1); 
	float z = (10.0 * ((GLfloat) rand ()) / ((GLfloat) RAND_MAX)) - 5.0;
...
void display(void)
{
	...
	else if (SingleMode)
	{
		...
		else
		{
			...
			std::string numships= "Presently number of ships are: ";
