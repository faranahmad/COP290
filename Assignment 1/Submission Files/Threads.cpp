		...
		for(int i=0;i<FinalBoard.GetNumberBalls();i++)
		{
			if (i != ballid)
			{
				...
				if(BallConsidered.GetRadius()+Vector_Of_Balls[i].GetRadius()>distance)
				{
					...
					BallConsidered.SetX(BallConsidered.GetX()+ux1/2);
					BallConsidered.SetY(BallConsidered.GetY()+uy1/2);
					

					Vector_Of_Balls[i].SetX(Vector_Of_Balls[i].GetX()+ux2/2);
					Vector_Of_Balls[i].SetY(Vector_Of_Balls[i].GetY()+uy2/2);
...
int main(int argc, char **argv)
{
	...
	FinalBoard=Board(1000,800,NumberOfBalls);
	// cout <<FinalBoard.Get
